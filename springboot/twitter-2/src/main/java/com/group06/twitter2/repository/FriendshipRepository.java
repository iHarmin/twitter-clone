...
public interface FriendshipRepository extends JpaRepository<Friendship, Integer> {
    ...
    @Query("SELECT f FROM Friendship f WHERE (f.user1 = :user OR f.user2 = :user) AND f.accepted = false")
    List<Friendship> findFriendRequestsByUser(@Param("user") Twitter2 user);

    @Query("SELECT f FROM Friendship f WHERE (f.user1 = :user OR f.user2 = :user) AND f.accepted = true")
